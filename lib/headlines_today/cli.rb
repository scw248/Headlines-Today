class HeadlinesToday::CLI
  ...
  def list_headlines
    HeadlinesToday::Scraper.new.make_headlines
    HeadlinesToday::Headline.all.each.with_index(1) do |headline, i|
      puts
      puts "#{i}. #{headline.name}"
      puts headline.source_url.to_s.colorize(:light_blue).underline
      puts
    end
  end
  ...
  def options
    ...
    while input != 'exit'
      ...
      if input.to_i > 0 && input.to_i <= HeadlinesToday::Headline.all.count
        puts
        puts HeadlinesToday::Headline.all[input.to_i - 1].story
      elsif input == 'list'
        list_headlines
      elsif input.to_i > HeadlinesToday::Headline.all.count
        puts
